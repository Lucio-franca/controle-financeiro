...
function Transacoes() {
  ...
  return (
    <div style={{ background: 'white', padding: '30px', borderRadius: '8px' }}>
      <h2 style={{ fontSize: '24px', marginTop: 0, marginBottom: '25px' }}>📋 Movimentações</h2>
      ...
      {transacoesFiltradas.length === 0 ? (
        <p style={{ textAlign: 'center', color: '#999', fontSize: '18px', padding: '40px' }}>Nenhuma movimentação registrada</p>
      ) : (
              ...
              <tr key={t.id} style={{ borderBottom: '1px solid #eee' }}>
                <td style={{ padding: '14px' }}>
                  <span style={{
                    ...
                    padding: '6px 12px', borderRadius: '4px', fontWeight: 'bold', fontSize: '14px'
                  }}>
                    {t.tipo === 'entrada' ? '📥 Entrada' : '📤 Saída'}
                  </span>
                </td>
                <td style={{ padding: '14px', fontSize: '15px' }}>{t.descricao}</td>
                <td style={{ padding: '14px', fontSize: '15px', color: '#667eea' }}>{getNomeCategoria(t)}</td>
                <td style={{ padding: '14px', fontWeight: 'bold', fontSize: '15px', color: t.tipo === 'entrada' ? '#2f9e44' : '#c92a2a' }}>
                ...
                <td style={{ padding: '14px', fontSize: '15px' }}>{new Date(t.data_transacao).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
