...
function Entradas({ onUpdate }) {
  ...
  return (
    <div style={{ background: 'white', padding: '30px', borderRadius: '8px' }}>
      <h2 style={{ fontSize: '24px', marginTop: 0, marginBottom: '20px' }}>📥 Entradas</h2>

      <div style={{ marginBottom: '25px', fontSize: '18px', fontWeight: 'bold', background: '#d3f9d8', padding: '15px', borderRadius: '6px', color: '#2f9e44' }}>
        Total de Entradas: R$ {total.toFixed(2)}
      </div>

      <button
        onClick={() => setMostraForm(!mostraForm)}
        style={{ padding: '14px 32px', background: '#51cf66', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', marginBottom: '30px', fontSize: '16px', fontWeight: 'bold' }}
      >
        ➕ Nova Entrada
      </button>
        ...
        <thead style={{ background: '#f5f5f5', borderBottom: '2px solid #ddd' }}>
          <tr>
            ...
            <th
              ...
              style={{ padding: '14px', textAlign: 'left', cursor: 'pointer', userSelect: 'none' }}
            >
              Data {ordenacao === 'desc' ? '↓' : '↑'}
            </th>
        ...
        <tbody>
          {transacoesFiltradas.map(t => (
            <tr key={t.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '14px', fontWeight: 'bold', color: '#2f9e44' }}>R$ {parseFloat(t.valor).toFixed(2)}</td>
