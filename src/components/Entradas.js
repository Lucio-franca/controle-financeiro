...
function Entradas({ onUpdate }) {
  ...
  return (
    <div style={{ background: 'white', padding: '30px', borderRadius: '14px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
      ...
      <h2 style={{ fontSize: '24px', marginTop: 0, marginBottom: '20px', fontWeight: '700', color: '#333' }}>📥 Entradas</h2>

      <div style={{ marginBottom: '25px', fontSize: '18px', fontWeight: '600', background: '#d3f9d8', padding: '18px', borderRadius: '10px', color: '#2f9e44', fontFamily: 'monospace', display: 'flex', justifyContent: 'space-between', border: '1px solid #51cf66' }}>
        <span>Total de Entradas:</span>
        <span>{formatarMoeda(total)}</span>
      </div>

      <button onClick={() => setMostraForm(!mostraForm)} className="btn-hover"
        style={{ padding: '12px 28px', background: '#51cf66', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', marginBottom: '30px', fontSize: '15px', fontWeight: '600' }}>
        ➕ Nova Entrada
      </button>
          ...
          <tbody>
            {transacoesFiltradas.map((t, idx) => (
              <tr key={t.id} className="row-appear"
                ...
                <td style={{ padding: '14px 16px', fontWeight: '600', color: '#2f9e44', fontSize: '14px', textAlign: 'right', fontFamily: 'monospace' }}>{formatarMoeda(parseFloat(t.valor))}</td>
